--[[
    deepseek.win v2.0
    Flight + Noclip + Speed + Gravity + Jump + Aimbot + ESP + Rainbow Cursor + Stream Chat
]]
...
chatList.Size = UDim2.new(1, -5, 1, -5)
chatList.Position = UDim2.new(0, 3, 0, 3)
...
chatLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(scrollToBottom)
...
local function addChatMessage(nick, text)
    ...
    local children = chatList:GetChildren()
    if #children > 30 then
...
local initialMessages = {
    ...
    {nick = "deepseek_win", text = "deepseek.win"},
    {nick = "ProHacker1337", text = "when aimbot?"},
